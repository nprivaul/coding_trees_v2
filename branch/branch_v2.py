...
#        logging.info(f"Logs are saved in {os.path.abspath(self.working_dir_full_path)}")
        logging.debug(f"Logs are saved in {os.path.abspath(self.working_dir_full_path)}")
        ...
        for cur_dim, cur_order in enumerate(order):
            for _ in range(int(cur_order)):
                try:
                    ...
                    #grads = torch.autograd.grad(y.sum(), x, grad_outputs=torch.ones_like(y), create_graph=True)
                except RuntimeError:
                ...
                # update y
                y = grads[cur_dim].detach().requires_grad_(True) # detach
        return y
    ...
    def code_to_function(self, code, x, T, patch, coordinate):
        ...
        if code[0] > 0:
            ...
            return self.nth_derivatives(
                code - 1, self.f_fun2(y, coordinate), y
            ).detach()

        return fun_val.detach()

    def code_to_function2(self, code, x, T, patch=0, coordinate=0):
        """
        ...
        # code of size dim
        den = torch.prod(torch.tensor(factorial(code), device=x.device, dtype=x.dtype))
        if len(code) == self.dim_in:
            return self.nth_derivatives(
                code, self.adjusted_phi(x, T, patch, coordinate), x
            ).detach()/den
#            ).detach()/np.prod(factorial(code))

        # positive code of size dim+1
        if len(code) == (self.dim_in+1):
            y = []
            for order in self.deriv_map:
                y.append(
                    self.nth_derivatives(
                        order, self.adjusted_phi(x, T, patch, coordinate), x
                    ).detach()
                )
            y = torch.stack(y[: self.n]).requires_grad_()

#            def f_der(y):
#                return self.nth_derivatives([code[self.dim_in]], self.f_fun2(y), y)
#            def f_der_circ_phi(x):
#                return f_der(self.adjusted_phi(x, T, patch, coordinate))
#            def der_f_der_circ_phi(x):
#                return self.nth_derivatives(code[:-1], f_der_circ_phi(x), x)
#
#            den = torch.prod(torch.tensor(factorial(code[:-1]), device=x.device, dtype=x.dtype))
#
#            return der_f_der_circ_phi(x).detach() / den

            # Force double precision for critical computations
            def f_der(y):
#                y = y.double() if y.is_cuda else y
                return self.nth_derivatives([code[self.dim_in]], self.f_fun2(y), y)

            def f_der_circ_phi(x):
#                x = x.double() if x.is_cuda else x
                return f_der(self.adjusted_phi(x, T, patch, coordinate))
...
#                x = x.double() if x.is_cuda else x
            ...
            # Compute denominator with higher precision
            den = torch.prod(torch.tensor(factorial(code[:-1]), 
                                          device=x.device, 
                                          dtype=torch.float64))

            return der_f_der_circ_phi(x).detach().to(x.dtype) / den.to(x.dtype)

            # return der_f_der_circ_phi(x).detach()/np.prod(factorial(code[:-1]))
    ...
    def gen_sample_batch(self, t, T, x, mask, H, code, patch, coordinate):
        ...
        if coordinate < 0:
            ...
            next_x = self.gen_bm(x, tau, nb_states, var=1).detach() # detach
        ...
        next_x = self.gen_bm(x, T - t, nb_states).detach() # detach
        ...
        if mask_now.any():
            tmp = (
                    ...
                    * (
                      ...
                      self.code_to_function2(code, next_x[:, mask_now], T[mask_now], patch, coordinate)
        ...
        ############################### for t + tau < T
        next_x = self.gen_bm(x, tau, nb_states).detach() # detach
        x_is_inside = self.is_x_inside(next_x)
...
#            print("ranges=",ranges)
            ...
            for fdb in L1:
                ...
                A = self.gen_sample_batch(
                    ...
                    mask_tmp,                 
                    H
                    *(self.dim_in + 1) * len(L1) /self.exponential_lambda/torch.exp(-self.exponential_lambda * tau),
                    # torch.ones_like(t),
                    np.append(code[:self.dim_in] - fdb.numpy(), 0),
                    patch,
                    coordinate,
                )
                #print("self.dim_in=",self.dim_in)
                #print("len(code)=",len(code))
                #print("code[self.dim_in]=",code[:self.dim_in])
                A = A * self.gen_sample_batch(
                    t + tau,
                    T,
                    next_x,
                    mask_tmp,
                    torch.ones_like(t),
                    np.append(fdb.numpy(),code[self.dim_in]+1), 
                    patch,
                    coordinate,
                )
                ans = ans.where(~mask_tmp, A)
            ...
            ranges = [torch.arange(int(jj)) for jj in np.append(code[:self.dim_in]+1,self.dim_in)]
...
#            idx = torch.searchsorted(torch.cumsum(abc,dim=0)*(self.dim_in + 1)/self.dim_in, unif)
...
#            print(len(L2))
            for fdb in L2:
                ...
                A = - (beta[ii]+1) * (code[ii]-beta[ii]+1) / 2 * self.gen_sample_batch(
                    ...
                    mask_tmp,
                    H
                    / self.exponential_lambda/torch.exp(-self.exponential_lambda * tau) * len(L2) , # torch.ones_like(t),
                    code[:self.dim_in] - beta.numpy() + np.eye(self.dim_in)[ii],
                    patch,
                    coordinate,
                )# /abc[idx_counter] # self.q(torch.tensor(code), beta, ii), 
                ...
                A = A * self.gen_sample_batch(
                    ...
                    mask_tmp,
                    torch.ones_like(t),
                    np.append(beta.numpy() + np.eye(self.dim_in)[ii],code[self.dim_in]+1), 
                    patch,
                    coordinate,
                )
#                print("A=",A)
#                A=A
    ...
    def gen_sample(self, patch, tx=None):
        ...
        for _ in range(batches):
            ...
            seed = int(time.time())  # changes every second
            torch.manual_seed(seed)

            for (idx, c) in zip(self.coordinate, self.code):
                xx.append(torch.cat((t[:, :1], x[:, :, 0].T), dim=-1).detach())
                self.code = np.array([0] * self.dim_in) 
...
#        print("yy=",yy)
    ...
    def gen_sample_for_p_batch(self, x, patch):
        ...
        tau = self.tau_lo + (self.tau_hi - self.tau_lo) * unif
        next_x = self.gen_bm(x.transpose(0, -1), tau.transpose(0, -1), x.shape[1], var=1).transpose(0, -1).detach() # detach
