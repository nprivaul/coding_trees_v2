...
class Net_cv(torch.nn.Module):
    ...
    exact_p_fun : function
        ...
        $$
        If `exact_p_fun` is not passed to Net_cv,
        Net_cv will use neural network approximation to solve
    ...
    code : numpy.ndarray
        Net_cv trains the network based on the `code = [c0, c1, ..., ck]`
    ...
    device : torch.device
        The device used by Net_cv, either cpu or cuda.
...
#        x_single,
        ...
        **kwargs,
        ...
        exact_p_fun : function, optional
            Default to be `None`,
            so that Net_cv will use neural network approximation to solve
        ...
        """
        super(Net_cv, self).__init__()
        self.problem_name = problem_name
        self.f_fun = f_fun
        self.cstar = cstar
        self.f_fun_cv = f_fun_cv
        self.exact_fun_cv = exact_fun_cv
        self.phi_fun = phi_fun
#        self.x_single = x_single
        self.phi_fun_cv = phi_fun_cv
        self.phi0 = phi0
        ...
        self.mechanism_tot_len = self.dim_in + self.dim_in * self.n ** 2 + sum(
        ...
        self.exponential_lambda = (
            ...
            else 1.0 # -math.log(.95)/self.delta_t
        )
    ...
    def adjusted_phi_cv(self, x, T, patch, coordinate):
        """
        Find the suitable terminal condition based on the value of patch
        when branch_patches=1, this function always outputs self.phi_fun_cv(x)
        ...
        """
        if patch == 0:
            return self.phi_fun_cv(x, coordinate)
        else:
            is_training = self.training
            self.eval()
            xx = torch.stack((T.reshape(-1), x.reshape(-1)), dim=-1)
    ...
    def code_to_function(self, code, x, T, patch, coordinate):
        ...
        if code[0] > 0:
            ...
            # print("yyy=",y);
            # print("self.n + self.dim_in=",self.n + self.dim_in); 
            # print("code here=",code); 
            y = torch.stack(y[: (self.dim_in + self.n)]).requires_grad_()
            return self.nth_derivatives(
    ...
    def code_to_function_cv(self, code, x, T, patch, coordinate):
        """
        Calculate the functional of tree based on code, x, and coordinate.
        ...
        """
        x = x.detach().clone().requires_grad_(True)
        fun_cv_val = torch.zeros_like(x[0])

        # negative code of size d
        if code[0] < 0:
            return self.nth_derivatives(
                -code - 1, self.adjusted_phi_cv(x, T, patch, coordinate), x
            ).detach()
        ...
        if code[0] > 0:
            ...
            for idx, order in enumerate(self.deriv_map):
                if self.zeta_map[idx] < 0:
                    ...
                    y.append(
                        self.nth_derivatives(
                            order, self.adjusted_phi_cv(x, T, patch, self.zeta_map[idx]), x
                        ).detach()
                    )
            # print("x=",x);
            # print("before",y);
            ...
            # print("after2",y);
            return self.nth_derivatives(
                code - 1, self.f_fun_cv(y, coordinate), y
            ).detach()

        return fun_cv_val.detach()
    ...
    def gen_sample_batch(self, t, T, x, mask, H, code, patch, coordinate):
        ...
        if (len(code) == self.dim_in) and (code == [-1] * self.dim_in).all():
            tmp = self.gen_sample_batch(
                ...
                np.array([1] * (self.dim_in + self.n)), # new 
                patch,
                coordinate,
            )
            ...
            if mask_tmp.any():
                ...
                tmp = self.gen_sample_batch(
                     ...
                     * H
                     ...
                     np.hstack((- code + code_increment, [1] * self.n)), # new
                     patch,
                     coordinate,
                )
                ...
                if mask_tmp.any():
                    A = fdb.coeff * self.gen_sample_batch(
                        ...
                        * H
                        / self.exponential_lambda
                        / torch.exp(-self.exponential_lambda * tau),
                        np.hstack(([1] * self.dim_in, np.array(fdb.lamb) + 1)), # new 
                        patch,
                        coordinate,
                    )
        ...
        # positive code of size n
            ...
            # positive code part 0 (new) 
            for k in range(self.dim_in):
                mask_tmp = mask_now * (idx == idx_counter)
                if mask_tmp.any():
                    # code_increment = np.zeros_like(self.deriv_map[i])
                    # code_increment[k] += 1
                    # A = self.gen_sample_batch_cv(
                    #     t + tau,
                    #     T,
                    #     next_x,
                    #     mask_tmp,
                ...
                for j in range(self.n):
                    for k in range(self.dim_in):
                        ...
                        if mask_tmp.any():
                            ...
                            # only code + 1 in the dimension j and l
                            code_increment = np.zeros_like(code)
                            code_increment[self.dim_in+i] += 1
                            code_increment[self.dim_in+j] += 1
                            tmp = self.gen_sample_batch(
                                t + tau,
                                T,
                                next_x,
            ...
            for k in range(self.nprime, self.n):
                for fdb in self.fdb_lookup[tuple(self.deriv_map[k])]:
                    ...
                    if mask_tmp.any():
                        ...
                        code_increment = np.zeros_like(code)
                        code_increment[self.dim_in+k] += 1
                        tmp = self.gen_sample_batch(
                            t + tau,
                            T,
                            next_x,
    ...
    def gen_sample_batch_cv(self, t, T, x, mask, H, code, patch, coordinate):
        """
        Recursive function to calculate E[ H(t, x, code) ].
        ...
        if mask_now.any():
            tmp = (
                    ...
                    * (
                        x_is_inside[mask_now] * survive_prob[mask_now]
                            * self.code_to_function_cv(code, next_x[:, mask_now], T[mask_now], patch, coordinate)
                        + (1 - x_is_inside[mask_now] * survive_prob[mask_now]) * self.phi0
                    )
            )
            ans[mask_now] = tmp
        ...
        # negative code of size d
            ...
            for fdb in L:
                mask_tmp = mask_now * (idx == idx_counter)
                if mask_tmp.any():
                    A = fdb.coeff * self.gen_sample_batch_cv(
                        t + tau,
                        T,
                        next_x,
                        mask_tmp,
    ...
    def gen_sample_cv(self, patch, tx=None):
        """
        ...
        vals = torch.linspace(self.x_lo, self.x_hi, states_per_batch, device=self.device)
        ...
        for _ in range(batches):
            ...
            if tx is None:
                ...
                # x_single_tensor = torch.as_tensor(self.x_single, device=self.device, dtype=torch.float32)
                # x = x_single_tensor.view(self.dim_in, 1, 1)
                # x = x.expand(self.dim_in, states_per_batch, self.nb_path_per_state)
                x = torch.zeros(
                    self.dim_in,
                    states_per_batch,
                    self.nb_path_per_state,
                    device=self.device,
                    dtype=unif.dtype,
                )
                fixed_dim=1
                # print("x bef exp=",x)
                x[fixed_dim, :, :] = vals.unsqueeze(-1)
                x = x.expand(self.dim_in, states_per_batch, self.nb_path_per_state)
            ...
            for (idx, c) in zip(self.coordinate, self.code):
                ...
                if torch.cuda.is_available():
                    current_seed_cuda = torch.cuda.get_rng_state()
                ...
                if torch.cuda.is_available():
                    torch.cuda.random.set_rng_state(current_seed_cuda)
                yy_tmp_cv = self.gen_sample_batch_cv(
                    t,
                    T,
                    x,
                    torch.ones_like(t),
                ...
                # print("yy_tmp_cv=",yy_tmp_cv)
                # let (lo, hi) be
                # (self.outlier_percentile, 100 - self.outlier_percentile)
                # percentile of yy_tmp_cv
                #
                # set the boundary as [lo-1000*(hi-lo), hi+1000*(hi-lo)]
                # samples out of this boundary is considered as outlier and removed
                # self.cstar = -1.0
                # print("self.f_fun_cv(1,0)=",self.f_fun_cv(1,0))
                #print(t.size())
                #print("t=",t)
                #print(x.size())
                #print("x=",x)
                #yy_tmp = yy_tmp + self.cstar * ( yy_tmp_cv - self.exact_fun_cv(t, x, T)[0])
                #print("yy_tmp=",yy_tmp)
                #print("yy_tmp_cv=",yy_tmp_cv)
                covariances = ((yy_tmp - yy_tmp.mean(dim=1, keepdim=True)) * (yy_tmp_cv - yy_tmp_cv.mean(dim=1, keepdim=True))).sum(dim=1) / (yy_tmp.shape[1] - 1)
                variances = ((yy_tmp_cv - yy_tmp_cv.mean(dim=1, keepdim=True)) * (yy_tmp_cv - yy_tmp_cv.mean(dim=1, keepdim=True))).sum(dim=1) / (yy_tmp.shape[1] - 1)
                variancemu = ((yy_tmp - yy_tmp.mean(dim=1, keepdim=True)) * (yy_tmp - yy_tmp.mean(dim=1, keepdim=True))).sum(dim=1) / (yy_tmp.shape[1] - 1)
                cstars = -covariances/variances
                rhostar = covariances/(variances*variancemu)**0.5
                print("cstar=",self.cstar)
                print("mean cstar=",cstars.mean())
                print("mean rhostar=",rhostar.mean())
                print("1-(rhostar)^2=",1-rhostar.mean()**2)
                #print("covariances=",covariances)
                #print("est cstars",cstars)
                # if (yy_tmp==yy_tmp_cv): print("yy_tmp==yy_tmp_cv");
                # yy_tmp = yy_tmp + ( yy_tmp_cv - torch.squeeze(self.exact_fun_cv(t, x, T,idx))) * cstars.unsqueeze(1) # Added "idx" coordinate 
                # yy_tmp = yy_tmp + self.cstar * ( yy_tmp_cv - torch.squeeze(self.exact_fun_cv(t, x, T,idx))) # Added "idx" coordinate 
                # print("cstars=",cstars)
                # print("yy_tmp=",yy_tmp)
                # print("yy_tmp_cv=",yy_tmp_cv)
                # print("x",x) 
                # print("self.exact_fun_cv(t, x, T,idx)=",self.exact_fun_cv(t, x, T,idx))
                yy_tmp = yy_tmp + ( yy_tmp_cv - self.exact_fun_cv(t, x, T,idx)) * cstars.unsqueeze(1) # Added "idx" coordinate 
                # yy_tmp = yy_tmp + cstars.mean() * ( yy_tmp_cv - self.exact_fun_cv(t, x, T,idx)) # Added "idx" coordinate 
                # yy_tmp = yy_tmp + cstars * ( yy_tmp_cv - torch.squeeze(self.exact_fun_cv(t, x, T,idx))) # Added "idx" coordinate 
                # print("t=",t)
                # print("x=",x)
                # print("yy_tmp=",yy_tmp)
                # print("yy_tmp_cv2=",torch.squeeze(self.exact_fun_cv(t, x, T,idx))) 
#                yy_tmp = yy_tmp_cv 
# print("x=",x) 
                # print("sol=",self.exact_fun_cv(t, x, T)[0])
                lo, hi = (
    ...
    def train_and_eval(self, debug_mode=False, return_dict=False, reuse_x=None, reuse_y=None):
        ...
        for p in range(self.patches):
            ...
            if self.train_for_u:
                ...
                start = time.time()
                if reuse_x is None:
                    x, y = self.gen_sample_cv(patch=p)
                else:
                    x, y = reuse_x, reuse_y
                self.print_msg(
