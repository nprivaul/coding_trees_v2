...
class AllenCahn2(Equation):
    """Allen-Cahn equation in PNAS paper doi.org/10.1073/pnas.1718942115"""
    def __init__(self, eqn_config):
        super(AllenCahn2, self).__init__(eqn_config)
        self.x_init = np.zeros(self.dim)
        self.sigma = np.sqrt(2.0)
    ...
    def g_tf(self, t, x):
        return -0.5-0.5 * tf.math.tanh(-0.5*tf.reduce_sum(x, 1, keepdims=True)/np.sqrt(self.dim))


class AllenCahn3(Equation):
    def __init__(self, eqn_config):
        super(AllenCahn3, self).__init__(eqn_config)
        self.x_init = np.zeros(self.dim)
        self.sigma = np.sqrt(2.0)
    ...
    def f_tf(self, t, x, y, z):
        return 0.1 * ( y - tf.pow(y, 3) )

    def g_tf(self, t, x):
        return tf.reduce_sum(tf.square(x), 1, keepdims=True)/self.dim

class AllenCahn4(Equation):
    """Allen-Cahn equation in PNAS paper doi.org/10.1073/pnas.1718942115"""
    def __init__(self, eqn_config):
        super(AllenCahn4, self).__init__(eqn_config)
        self.x_init = np.zeros(self.dim)
        self.sigma = 1.0
#        self.sigma = np.sqrt(2.0)
...
class Exp(Equation):
    """Exponential example"""
    def __init__(self, eqn_config):
        super(Exp, self).__init__(eqn_config)
        shape = [3] * self.dim
        # array = np.zeros(self.dim)
        array = tf.zeros(self.dim)
        self.x_init = tf.tensor_scatter_nd_update(array, indices=[[0]], updates=[0.0])
        self.sigma = np.sqrt(2.0)
    ...
    def f_tf(self, t, x, y, z):
        return 6 + 4*tf.exp(-y) - 10*tf.exp(-y/2) + tf.exp(y/2) - tf.exp(y)

    def g_tf(self, t, x):
        return  2*tf.math.log(1+1/(1+tf.exp(tf.reduce_sum(x,1,keepdims=True) / np.sqrt(self.dim))))
